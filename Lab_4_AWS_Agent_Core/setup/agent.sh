...
ensure_credentials() {
    if [ ! -f ".mcp-credentials.json" ]; then
        ...
        echo ""
        echo "Copy credentials from MCP server deployment:"
        echo "  cp /path/to/mcp-server/.mcp-credentials.json ."
        echo ""
        echo "See mcp-credentials.json.example for the required format."
        exit 1
    fi
}
...
case "${1:-help}" in
    setup)
        ...
        echo ""
        echo "Next steps:"
        echo "  1. Copy credentials: cp /path/to/mcp-credentials.json .mcp-credentials.json"
        echo "     See mcp-credentials.json.example for the required format."
        echo ""
        echo "  Local testing:"
        echo "    ./agent.sh start              # Start on port 8080"
        echo "    ./agent.sh test-maintenance   # Test Maintenance Agent routing"
        echo "    ./agent.sh test-operations    # Test Operations Agent routing"
        echo ""
        echo "  Cloud deployment:"
        echo "    ./agent.sh configure          # Create AWS config (accept defaults)"
        echo "    ./agent.sh deploy             # Deploy to AgentCore Runtime"
        echo "    ./agent.sh invoke-cloud       # Test deployed agent"
        ;;
